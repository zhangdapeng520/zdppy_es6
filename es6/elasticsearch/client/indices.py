...
class IndicesClient(NamespacedClient):
    ...
    def analyze(self, index=None, body=None, params=None):
        """
        对文本执行分析过程，并返回文本的词块分类。
        `<http://www.elastic.co/guide/en/elasticsearch/reference/current/indices-analyze.html>`_

        :arg index: 索引名称 to scope the operation
        :arg body: Define analyzer/tokenizer parameters and the text on which the analysis should be performed
        :arg format: Format of the output, default 'detailed', valid choices are: 'detailed', 'text'
    ...
    def flush(self, index=None, params=None):
        ...
        :arg index: 以逗号分隔的索引名称列表；使用 `_all`或空字符串表示所有索引
        :arg allow_no_indices: 如果通配符索引表达式解析为无具体索引，是否忽略。(这包括 `_all`字符串或未指定索引时）
        :arg expand_wildcards: 是否将通配符表达式扩展为开放、封闭或两者兼有的具体索引，默认为 "开放"，有效选项为："开放"、"封闭"、"无"、"全部"： 打开"、"关闭"、"无"、"全部., default 'open', valid choices are: 'open', 'closed', 'none', 'all'
        :arg force: Whether a flush should be forced even if it is not necessarily needed ie. if no changes will be committed to the index. This is useful if transaction log IDs should be incremented even if no uncommitted changes are present. (This setting can be considered as internal)
        :arg ignore_unavailable: 指定的具体索引在不可用（丢失或关闭）时是否应被忽略
        :arg wait_if_ongoing: If set to true the flush operation will block until the flush can be executed if another flush operation is already executing. The default is true. If set to false the flush will be skipped iff if another flush operation is already running.
        """
        return self.transport.perform_request('POST', _make_path(index,
                                                                 '_flush'), params=params)

    @query_params('master_timeout', 'timeout', 'update_all_types', 'wait_for_active_shards')
    def create(self, index, body=None, params=None):
        ...
        :arg timeout: 显式操作超时
        :arg update_all_types: Whether to update the mapping for all fields with the same name across all types or not
        :arg wait_for_active_shards: Set the number of active shards to wait for before the operation returns.
        """
    ...
    def get(self, index, feature=None, params=None):
        ...
        :arg allow_no_indices: Ignore if a wildcard expression resolves to no concrete indices (default: false)
        :arg expand_wildcards: Whether wildcard expressions should get expanded to open or closed indices (default: open), default 'open', valid choices are: 'open', 'closed', 'none', 'all'
        :arg flat_settings: Return settings in flat format (default: false)
        :arg ignore_unavailable: Ignore unavailable indexes (default: false)
        :arg include_defaults: Whether to return all default setting for each of the indices., default False
    ...
    def open(self, index, params=None):
        ...
        :arg index: 索引名称
        :arg allow_no_indices: 如果通配符索引表达式解析为无具体索引，是否忽略。(这包括 `_all`字符串或未指定索引时）
        :arg expand_wildcards: Whether to expand wildcard expression to concrete indices that are open, closed or both., default 'closed', valid choices are: 'open', 'closed', 'none', 'all'
        :arg ignore_unavailable: 指定的具体索引在不可用（丢失或关闭）时是否应被忽略
        :arg master_timeout: Specify timeout for connection to master
        :arg timeout: 显式操作超时
        """
    ...
    def delete(self, index, params=None):
        ...
        :arg index: A comma-separated list of indices to delete; use `_all` or `*` string to delete all indices
        :arg allow_no_indices: Ignore if a wildcard expression resolves to no concrete indices (default: false)
        :arg expand_wildcards: Whether wildcard expressions should get expanded to open or closed indices (default: open), default 'open', valid choices are: 'open', 'closed', 'none', 'all'
        :arg ignore_unavailable: Ignore unavailable indexes (default: false)
        :arg master_timeout: Specify timeout for connection to master
    ...
    def exists_template(self, name, params=None):
        ...
        :arg flat_settings: Return settings in flat format (default: false)
        :arg local: Return local information, do not retrieve the state from master node (default: false)
        :arg master_timeout: 显式操作超时 for connection to master
            node
        """
    ...
    def validate_query(self, index=None, doc_type=None, body=None, params=None):
        ...
        """
        return self.transport.perform_request('GET', _make_path(index,
                                                                doc_type, '_validate', 'query'), params=params,
                                              body=body)
